<?php get_header(); ?>
                ...
                <div id="container">    
                        <div id="content">
<p class="notice">Showing Google Search results from <a href="<?php bloginfo('url'); ?>" title="<?php bloginfo('name'); ?>"><?php bloginfo('name'); ?></a>. <a href="http://www.googlesyndicatedsearch.com/u/newmexicostate?q=<?php echo get_query_var('s'); ?>" target="_blank">Search NMSU</a> for more results.</p>
<?php if( get_query_var('search_method') == 'google' ) : ?>
<!-- Ripped straight from http://www.nmsu.edu/search_results.php -->
<script src="http://www.google.com/jsapi" type="text/javascript"></script>
                    <script type="text/javascript">
                        google.load('jquery', '1');
                        google.load('search', '1', {language : 'en'});
                        google.setOnLoadCallback(function(){
                            var customSearchControl = new google.search.CustomSearchControl('003174599515884405122:a7gnjdfior4');
                            customSearchControl.setResultSetSize(google.search.Search.FILTERED_CSE_RESULTSET);
                            customSearchControl.draw('cse');
                            $(".gsc-input").val("<?php echo 'site:' . get_bloginfo('url') . ' ' . get_query_var('s'); ?>");//insert into search field requested search text
...
<?php else: ?>                       
        <?php if ( have_posts() ) : ?>
                                    ...
                                    <h1 class="page-title"><?php _e( 'Search Results for: ', 'NMSU' ); ?><span><?php the_search_query(); ?></span></h1>
    ...
    <?php else : ?>
                                    ...
                                    <div id="post-0" class="post no-results not-found">
                                            <h2 class="entry-title"><?php _e( 'Nothing Found', 'NMSU' ) ?></h2>
                                            <div class="entry-content">
                                                    <p><?php _e( 'Sorry, but nothing matched your search criteria. Please try again with some different keywords.', 'NMSU' ); ?></p>
            <?php get_search_form(); ?>                                             
                                            </div><!-- .entry-content -->
                                    </div>
